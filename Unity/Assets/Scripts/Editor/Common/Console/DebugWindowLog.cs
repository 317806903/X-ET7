...
namespace XGame
{
    public class DebugWindowLog : DebugWindowBase
    {
        ...
        private Regex _atFile = new Regex(@"at (.*) in (.*)\:(\d+)");
        ...
        private string StacktraceWithHyperlinks(string stacktraceText, out string file, out int line)
        {
            var stringBuilder = new StringBuilder();
             ...
             else if (this._atFile.IsMatch(stacktraceText))
             {
                 var result = this._atFile.Match(stacktraceText);
