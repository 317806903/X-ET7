...
namespace ET.Client
{
    public static class UIGuideHelper_StaticMethod
    {
        ...
        public static async ETTask ShowVideo(Scene scene, string tutorialCfgId)
        {
            ...
            {
                await UIManagerHelper.GetUIComponent(scene).ShowWindowAsync<DlgTutorials>(new ShowData_DlgTutorials(){
                    menuType = TutorialMenuType.GameTips,
                });
            }
            else
            {
        ...
        {
            EventSystem.Instance.Publish(scene, new ClientEventType.HideBattleTowerHUD());
            await ETTask.CompletedTask;
        }

        public static async ETTask ShowBattleTowerReady(Scene scene, bool isShow)
        {
            await ET.Client.UIManagerHelper.ShowBattleUIReady(scene, isShow);
        }

        public static async ETTask ShowBattleTowerQuit(Scene scene, bool isShow)
        {
            await ET.Client.UIManagerHelper.ShowBattleUIQuit(scene, isShow);
        }
