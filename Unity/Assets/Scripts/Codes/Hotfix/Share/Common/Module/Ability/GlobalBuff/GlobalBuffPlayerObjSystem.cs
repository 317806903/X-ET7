    ...
    public static class GlobalBuffPlayerObjSystem
    {
                ...
                if (self.model.TickTime[i] > 0)
                {
                    ...
                    while (newCount > lastCount)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            actionGameContext.playerId = self.playerId;
                            actionGameContext.teamFlagType = self.teamFlagType;
        ...
        public static void TrigEvent(this GlobalBuffPlayerObj self, ET.AbilityConfig.GlobalBuffTriggerEvent abilityGameMonitorTriggerEvent, ref ActionGameContext actionGameContext)
        {
            ...
            if (bPass)
            {
                ...
                if (actionGameContextNew.playerId == (long)ET.PlayerId.PlayerNone)
                {
                ...
                {
                    if (abilityGameMonitorTriggerEvent == GlobalBuffTriggerEvent.GamePlayTowerDefense_Status_SkillReady)
                    {
                        Unit cameraPlayerUnit = ET.GamePlayHelper.GetCameraPlayerUnit(self.DomainScene(), self.playerId);
                        if (cameraPlayerUnit != null)
                        {
                            actionGameContextNew.unitId = cameraPlayerUnit.Id;
                        }
                    }
                    else
                    {
                ...
                foreach (string actionId in self.model.MonitorTriggers.ActionIds)
                {
