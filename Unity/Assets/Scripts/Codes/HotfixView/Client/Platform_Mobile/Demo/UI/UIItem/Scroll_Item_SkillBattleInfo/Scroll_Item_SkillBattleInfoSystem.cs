...
namespace ET.Client
{
	...
	public static class Scroll_Item_SkillBattleInfoSystem
	{
        ...
        public static void _UpdateSkillInfo_CD(this Scroll_Item_SkillBattleInfo self, ET.Ability.SkillObj skillObj)
        {
	        ...
	        if (skillObj.isInitWhenClient == false)
	        {
		        skillObj.isInitWhenClient = true;
		        self.curCDCountDownTime = (long)(skillObj.cdCountDown*1000) + TimeHelper.ClientNow();
	        }
	        long leftTime = self.curCDCountDownTime - TimeHelper.ClientNow();
	        if (leftTime < 0)
	        {
		        leftTime = 0;
	        }

	        float leftTimeFloat = leftTime / 1000f;
	        self.EImage_CDImage.fillAmount = 1 - leftTimeFloat / skillObj.cdTotal;
        }
        ...
        public static void _UpdateSkillInfo_EnergyCD(this Scroll_Item_SkillBattleInfo self, ET.Ability.SkillObj skillObj)
        {
	        ...
	        if (skillConsumeCfg.ConsumeCommonEnergy > 0)
	        {
		        ...
		        float totalCommonEnergy = ET.Ability.UnitHelper.GetSkillCommonEnergyFullNum(unit);
	        ...
	        {
		        float totalEnergy = skillObj.totalEnergyNum;
