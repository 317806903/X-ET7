    ...
    public override void OnInspectorGUI()
    {
        if (EditorApplication.isPlaying)
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    DOTweenSequence doTweenSequence = target as DOTweenSequence;
                    doTweenSequence.DOKill();
                    UnityEvent onComplete = doTweenSequence.GetCompleteAction();
                    doTweenSequence.SetCompleteAction(new UnityEvent());
                    doTweenSequence.AddCompleteAction(() =>
                    {
                        doTweenSequence.SetCompleteAction(onComplete);
                    }, true);
...
public class DOTweenSequence : MonoBehaviour
{
    ...
    public void AddCompleteAction(Action completeAction, bool needClear)
    {
        if (needClear)
        {
            this.m_OnComplete.RemoveAllListeners();
        }
        this.m_OnComplete.AddListener(()=>
        {
            completeAction?.Invoke();
        });
    }

    public UnityEvent GetCompleteAction()
    {
        return m_OnComplete;
    }

    public void SetCompleteAction(UnityEvent onComplete)
    {
                ...
                case DOTweenType.DOScale:
                    {
                        ...
                        var startValue = UseFromValue ? (Vector3)FromValue : transform.localScale;
                        if (reverse)
                        {
                            (targetValue, startValue) = (startValue, targetValue);
                        }
                ...
                case DOTweenType.DORotate:
                    {
                        ...
                        var startValue = UseFromValue ? (Vector3)FromValue : transform.eulerAngles;
                        if (reverse)
                        {
                            (targetValue, startValue) = (startValue, targetValue);
                        }
                        ...
                        result = transform.DORotate(targetValue, duration, RotateMode.FastBeyond360);
                    }
                    break;
                case DOTweenType.DOLocalRotate:
                    {
                        ...
                        var startValue = UseFromValue ? (Vector3)FromValue : transform.localEulerAngles;
                        if (reverse)
                        {
                            (targetValue, startValue) = (startValue, targetValue);
                        }
                        ...
                        if (SpeedBased)
                            duration = GetEulerAnglesAngle(targetValue, startValue) / this.DurationOrSpeed;
                        result = transform.DOLocalRotate(targetValue, duration, RotateMode.FastBeyond360);
                    }
                    break;
                case DOTweenType.DOAnchorPos:
                    {
