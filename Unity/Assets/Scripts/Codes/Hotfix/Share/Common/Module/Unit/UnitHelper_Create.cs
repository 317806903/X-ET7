...
namespace ET.Ability
{
    ...
    public static class UnitHelper_Create
    {
        ...
        public static Unit CreateWhenServer_SkillCasterUnit(Scene scene, long playerId, int level, float3 position, float3 forward)
        {
            string unitCfgId = "Unit_PlayerSkillCaster";
            UnitType unitType = UnitType.SkillCasterUnit;
        ...
        /// <returns></returns>
        public static Unit CreateWhenServer_ActorUnit(Scene scene, UnitType unitType, string unitCfgId, int level, float3 position, float3 forward, string aiCfg)
        {
            UnitComponent unitComponent = GetUnitComponent(scene);
            Unit unit = CreateWhenServer_Common_Before(unitComponent, unitCfgId, unitType, position, forward);
            unit.level = level;
        ...
        /// <param name="pathfindingMapName"></param>
        /// <returns></returns>
        public static Unit CreateWhenServer_CallActorUnit(Scene scene, Unit unitCaster, ActionCfg_CallActor actionCfgCallActor, SelectHandle selectHandle, ref ActionContext actionContext, float3? position)
        {
            float3 newPosition;
            float3 newForward = new float3(0, 0, 1);
            if (actionCfgCallActor.CallActorPositionType == CallActorPositionType.ByParent)
            {
                (newPosition, newForward) = UnitHelper.GetNewNodePosition(unitCaster, actionCfgCallActor.OffSetInfo);
            }
            else
            {
            ...
            UnitType unitType = UnitType.ActorUnit;
            if (actionCfgCallActor.CallActorUnitType == CallActorUnitType.NPC)
            {
                unitType = UnitType.NPC;
            }
            Unit actorUnit = CreateWhenServer_ActorUnit(scene, unitType, actionCfgCallActor.UnitId, unitLevel, newPosition, newForward, aiCfgId);
        ...
        public static Unit CreateWhenServer_CallGlobalActorUnit(Scene scene, ActionGameCfg_CreateGlobalUnit actionGameCfgCreateGlobalUnit, ref ActionContext actionContext, float3? position)
        {
            int unitLevel = actionGameCfgCreateGlobalUnit.Level;

            string aiCfgId = actionGameCfgCreateGlobalUnit.UnitAiCfg;

            UnitType unitType = UnitType.ActorUnit;
            if (actionGameCfgCreateGlobalUnit.CallActorUnitType == CallActorUnitType.NPC)
            {
                unitType = UnitType.NPC;
            }
            ...
            float3 newForward = new float3(0, 0, 1);
            Unit actorUnit = CreateWhenServer_ActorUnit(scene, unitType, actionGameCfgCreateGlobalUnit.UnitId, unitLevel, newPosition, newForward, aiCfgId);
            GamePlayHelper.AddUnitPathfinding(actorUnit);
        ...
        public static void SetUnitNumeric(NumericComponent numericComponent, string propertyType, int unitLevel)
        {
            ...
            numericComponent.SetAsInt(NumericType.DamageReliefWhenPhysicalBase, unitPropertyCfg.DamageReliefWhenPhysicalBase);
            numericComponent.SetAsInt(NumericType.DamageReliefWhenMagicBase, unitPropertyCfg.DamageReliefWhenMagicBase);
        }
        ...
        public static void CopyUnitNumeric(NumericComponent numericComponentFrom, NumericComponent numericComponentTo, bool isNeedCopyAll = true)
        {
            if (isNeedCopyAll)
            {
                foreach (var numeric in numericComponentFrom.NumericDic)
                {
                    int numericKey = numeric.Key;
                    long numericValue = numeric.Value;
                    numericComponentTo.SetNoEvent(numericKey, numericValue, false);
                }
