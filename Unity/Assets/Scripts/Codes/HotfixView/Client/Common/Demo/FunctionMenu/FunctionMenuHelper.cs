...
namespace ET.Client
{
	...
	{
        public static async ETTask ChkNeedShowFunctionMenuGuide(Scene scene)
        {
            ...
            await _ChkNeedShowFunctionMenuGuide(scene, false);
        }

        public static async ETTask _ChkNeedShowFunctionMenuGuide(Scene scene, bool needWaitFrame = false)
        {
            if (scene.IsDisposed)
            {
                return;
            ...
            if (openningList.Count > 0)
            {
                ...
                Action<Scene> doGuile = async (scene) =>
                {
                    ...
                    if (string.IsNullOrEmpty(functionMenuCfg.UIGuideConfigFileName))
                    {
                        ...
                        await ET.Client.FunctionMenu._ChkNeedShowFunctionMenuGuide(scene);
                    }
                    else
                    {
                        await ET.Client.UIGuideHelper.DoUIGuide(scene, functionMenuCfg.UIGuideConfigFileName, functionMenuCfg.Priority, 0, async (scene) =>
                        {
                            ...
                            ET.Client.FunctionMenu._ChkNeedShowFunctionMenuGuide(scene, true).Coroutine();
                        });
                    }
                };
