// #ifdef DM_NEW_CUSTOM_LIGHTING_INCLUDED
// 	#undef DM_NEW_CUSTOM_LIGHTING_INCLUDED
// #endif 
#ifndef DM_NEW_CUSTOM_LIGHTING_INCLUDED
#define DM_NEW_CUSTOM_LIGHTING_INCLUDED
...
*/
void DM_MainLight_float (out float3 Direction, out float3 Color, out float DistanceAtten){
...
*/
void DM_Shadowmask_half (float2 lightmapUV, out half4 Shadowmask){
	#ifdef SHADERGRAPH_PREVIEW
...
*/
void DM_MainLightShadows_float (float3 WorldPos, half4 Shadowmask, out float ShadowAtten){
	#ifdef SHADERGRAPH_PREVIEW
		ShadowAtten = 1;
	#else
...
void DM_MainLightShadows_float (float3 WorldPos, out float ShadowAtten){
	DM_MainLightShadows_float(WorldPos, half4(1,1,1,1), ShadowAtten);
}

float4 GetShadowPositionHClip(float3 WorldPos)
{
...
void DM_MainLightTerrainShadows_float (float3 WorldPos, half4 Shadowmask, out float ShadowAtten){
	#ifdef SHADERGRAPH_PREVIEW
		ShadowAtten = 1;
	#else
		float4 shadowCoord = TransformWorldToShadowCoord(WorldPos);
		ShadowAtten = MainLightTerrainShadow(shadowCoord, WorldPos, Shadowmask, _MainLightOcclusionProbes, GetShadowPositionHClip(WorldPos));
	#endif
}

void DM_MainLightTerrainShadows_float (float3 WorldPos, out float ShadowAtten){
	DM_MainLightTerrainShadows_float(WorldPos, half4(1,1,1,1), ShadowAtten);
}
...
*/
void DM_AmbientSampleSH_float (float3 WorldNormal, out float3 Ambient){
	#ifdef SHADERGRAPH_PREVIEW
...
*/
void DM_MixFog_float (float3 Colour, float Fog, out float3 Out){
	#ifdef SHADERGRAPH_PREVIEW
		Out = Colour;
	#else
...
- For a PBR/Lit Graph, these keywords are already handled for you.
*/
void DM_AdditionalLights_float(float3 SpecColor, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, half4 Shadowmask,
...
void DM_AdditionalLights_float(float3 SpecColor, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, 
							out float3 Diffuse, out float3 Specular) {
DM_AdditionalLights_float(SpecColor, Smoothness, WorldPosition, WorldNormal, WorldView, half4(1,1,1,1), Diffuse, Specular);
}
...
*/
#ifndef SHADERGRAPH_PREVIEW
float DM_ToonAttenuation(int i, float3 positionWS, float pointBands, float spotBands){
...
- For a PBR/Lit Graph, these keywords are already handled for you.
*/
void DM_AdditionalLightsToon_float(float3 SpecColor, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, half4 Shadowmask,
	...
	for (int i = 0; i < pixelLightCount; ++i) {
		...
		*/
		...
		if (PointLightBands <= 1 && SpotLightBands <= 1){
			...
			// Multiple bands :
			diffuseColor += light.color * light.shadowAttenuation * DM_ToonAttenuation(i, WorldPosition, PointLightBands, SpotLightBands);
		}
...
void DM_AdditionalLightsToon_float(float3 SpecColor, float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView,
						float PointLightBands, float SpotLightBands,
						out float3 Diffuse, out float3 Specular) {
DM_AdditionalLightsToon_float(SpecColor, Smoothness, WorldPosition, WorldNormal, WorldView, half4(1,1,1,1),
	PointLightBands, SpotLightBands,Diffuse, Specular);
}

#endif // DM_NEW_CUSTOM_LIGHTING_INCLUDED
