...
namespace ET.Client
{
    public static class HealthBarTowerComponentSystem
    {
        ...
        public static async ETTask Init(this HealthBarTowerComponent self, bool isShowWhenFull)
        {
            ...
            self.go = HealthBarGo;
            HealthBarGo.transform.SetParent(gameObjectShowComponent.GetEffectResScaleRoot().transform);
            ...
            float height = ET.Ability.UnitHelper.GetBodyHeight(self.GetUnit()) + clientResScale * 2f;
            HealthBarGo.transform.position = (Vector3)self.GetUnit().GetUnitClientPos() + new Vector3(0, height, 0);
            ...
            self.isShowWhenFull = isShowWhenFull;
        ...
        public static void UpdateHealth(this HealthBarTowerComponent self, bool isInit)
        {
            ...
            {
                if (self.isShowWhenFull)
                {
                    self.go.SetActive(true);
                    self.isActivity = true;
                }
                else if (isInit || self.isActivity)
                {
                    self.go.SetActive(false);
                    self.isActivity = false;
                }
