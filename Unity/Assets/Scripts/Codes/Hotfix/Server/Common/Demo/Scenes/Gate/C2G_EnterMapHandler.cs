...
namespace ET.Server
{
	...
	public class C2G_EnterMapHandler : AMRpcHandler<C2G_EnterMap, G2C_EnterMap>
	{
		protected override async ETTask Run(Session session, C2G_EnterMap request, G2C_EnterMap response)
		{
			...
			playerStatusComponent.RoomTypeInfo = ET.GamePlayHelper.GetRoomTypeInfo(RoomType.Normal, SubRoomType.NormalSingleMap, -1, -1, PVELevelDifficulty.Easy, gamePlayBattleLevelCfgId);
			playerStatusComponent.PlayerStatus = PlayerStatus.Battle;
			playerStatusComponent.MapScale = gamePlayBattleLevelCfg.MapScale;
			...
			TransferHelper.EnterMap(startSceneConfig.InstanceId, player.Id, player.Level, gamePlayBattleLevelCfgId, gamePlayBattleLevelCfg.MapScale).Coroutine();
