    ...
    public static class HealthBarNormalManagerSystem
    {
        ...
        public static async ETTask Init(this HealthBarNormalManager self)
        {
            ...
            while (gamePlayComponent == null || gamePlayComponent.IsDisposed)
            {
                ...
                gamePlayComponent = ET.Client.GamePlayHelper.GetGamePlay(self.DomainScene());
            }
            ...
            self.meshLocalPosition = self.healthBar.localPosition * clientResScale;
            self.meshSize = self.healthBar.localScale * clientResScale;
        }
        ...
        public static void ResetRendererData_GPUInstance(this HealthBarNormalManager self)
        {
            ...
            for (int j = self.curDelayHpPers.Length - 1; j >= 0; j--)
            {
                self.curDelayHpPers[j] = 0;
            }

            for (int j = self.curHpPers.Length - 1; j >= 0; j--)
            {
                self.curHpPers[j] = 0;
            }
