    ...
    public static class SkillComponentSystem
    {
        ...
        public class SkillComponentAwakeSystem: AwakeSystem<SkillComponent>
        {
            protected override void Awake(SkillComponent self)
            {
                ...
                self.sortPrioritySkillObjsWhenBlock = new();
        ...
        public class SkillComponentDestroySystem: DestroySystem<SkillComponent>
        {
            protected override void Destroy(SkillComponent self)
            {
                ...
                self.sortPrioritySkillObjsWhenBlock?.Clear();
        ...
        public static void SetCommonEnergyFull(this SkillComponent self)
        {
            ...
            int totalCommonEnergy = ET.Ability.UnitHelper.GetSkillCommonEnergyFullNum(unit);

            self.curCommonEnergyNum = totalCommonEnergy;
        }
        ...
        {
            return ET.Ability.UnitHelper.GetSkillCommonEnergyFullNum(self.GetUnit());
        }
        ...
        public static void DealCurCommonEnergyNumByTime(this SkillComponent self, float fixedDeltaTime)
        {
            ...
            int totalCommonEnergy = ET.Ability.UnitHelper.GetSkillCommonEnergyFullNum(unit);
            if (self.curCommonEnergyNum >= totalCommonEnergy)
            {
                return;
            }

            self.curCommonEnergyNum = math.clamp(self.curCommonEnergyNum + restoreCommonEnergyByTime * fixedDeltaTime, 0, totalCommonEnergy);
        }
        ...
        public static void DealCurCommonEnergyNumByWave(this SkillComponent self)
        {
            ...
            self.curCommonEnergyNum = math.clamp(self.curCommonEnergyNum + restoreCommonEnergyByWave, 0, totalCommonEnergy);
        }

        public static (bool ret, string msg, SkillObj skillObj) LearnSkill(this SkillComponent self, string skillCfgId, int skillLevel, SkillSlotType skillSlotType)
        {
            ...
            self.ReSortPrioritySkillObjsWhenBlock();

            skillObj.DealLearnActionIds().Coroutine();

            self.NoticeClient();

            return (true, "", skillObj);
        }
        ...
        public static void ReSortPrioritySkillObjs(this SkillComponent self)
        {
            self.sortPrioritySkillObjs.Clear();
            List<long> skillObjs;
            if (self.skillSlotType2SkillObjs.TryGetValue(SkillSlotType.InitiativeSkill, out skillObjs))
            {
                for (int i = 0; i < skillObjs.Count; i++)
                {
                    long skillId = skillObjs[i];
        ...
        public static void ReSortPrioritySkillObjsWhenBlock(this SkillComponent self)
        {
            self.sortPrioritySkillObjsWhenBlock.Clear();
            List<long> skillObjs;
            if (self.skillSlotType2SkillObjs.TryGetValue(SkillSlotType.BlockSkill, out skillObjs))
            {
                for (int i = 0; i < skillObjs.Count; i++)
                {
                    long skillId = skillObjs[i];
                    ...
                    if (string.IsNullOrEmpty(skillObj.model.TimelineId) == false)
                    {
                        self.sortPrioritySkillObjsWhenBlock.Add(skillId);
                    }
                }
            }
            if (self.skillSlotType2SkillObjs.TryGetValue(SkillSlotType.InitiativeSkill, out skillObjs))
        ...
        public static (float, SkillObj, int) GetSkillAttackDis(this SkillComponent self, HashSet<string> excludeSkillCfgIds, bool isGetMaxDisSkill)
        {
            float disOut = 0;
            SkillObj skillObjOut = null;
            int skillCount = self.sortPrioritySkillObjs.Count;
            ...
            for (int i = 0; i < skillCount; i++)
            {
                ...
                SkillObj skillObj = self.GetChild<SkillObj>(skillId);
                if (excludeSkillCfgIds != null && excludeSkillCfgIds.Contains(skillObj.skillCfgId))
                {
                    continue;
                }
                var result = skillObj.ChkCanUseSkill();
                if (result.ret)
                {
                    if (isGetMaxDisSkill == false)
                    {
                        float disTmp = skillObj.GetSkillDis();
                        disTmp *= resScale;

                        return (disTmp, skillObj, skillCount);
                    }
                    else
                    {
                        float disTmp = skillObj.GetSkillDis();
                        ...
                        {
                            disOut = disTmp;
                            skillObjOut = skillObj;
                        }
                    }
                }
            }

            if (disOut == 0 && skillObjOut == null)
            {
                disOut = self.GetMaxSkillDis(SkillSlotType.NormalAttack);
            }
            else
            {
                disOut *= resScale;
            ...
            return (disOut, skillObjOut, skillCount);
        }

        public static (float, SkillObj) GetSkillAttackDisWhenBlock(this SkillComponent self, HashSet<string> excludeSkillCfgIds)
        {
            float resScale = ET.Ability.UnitHelper.GetGameResScale(self.DomainScene());
            float dis = 0;
            for (int i = 0; i < self.sortPrioritySkillObjsWhenBlock.Count; i++)
            {
                long skillId = self.sortPrioritySkillObjsWhenBlock[i];
                SkillObj skillObj = self.GetChild<SkillObj>(skillId);
                if (excludeSkillCfgIds != null && excludeSkillCfgIds.Contains(skillObj.skillCfgId))
                {
                    continue;
                ...
                else
                {
                    if (self.skillObjs2SkillSlotType[skillId] == SkillSlotType.NormalAttack ||
                        self.skillObjs2SkillSlotType[skillId] == SkillSlotType.BlockSkill)
                    {
