...
namespace ET.Client
{
	...
	public static class DlgBattleTowerNoticeSystem
	{
		...
		public static async ETTask ShowWindow(this DlgBattleTowerNotice self, ShowWindowData contextData = null)
		{
			...
			await self.ShowOrHide(false);
		}

		public static async ETTask ShowOrHide(this DlgBattleTowerNotice self, bool isShow)
		{
			if (isShow)
			{
				if (self.View.EG_NoticeRectTransform.gameObject.activeSelf)
				{
					return;
				}
				...
				self.View.EG_NoticeRectTransform.SetVisible(true);
			}
			else
			{
				self.View.EG_NoticeRectTransform.SetVisible(false);
			}
		}
