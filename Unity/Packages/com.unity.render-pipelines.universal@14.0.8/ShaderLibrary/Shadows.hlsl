...
TEXTURE2D_SHADOW(_MainLightSelfShadowmapTexture);
SAMPLER_CMP(sampler_MainLightSelfShadowmapTexture);

TEXTURE2D_SHADOW(_DMDistanceTexture);
SAMPLER(sampler_DMDistanceTexture);

TEXTURE2D_SHADOW(_TerrainDepthTexture);
SAMPLER(sampler_TerrainDepthTexture);
...
real SampleTerrainShadowmap(TEXTURE2D_SHADOW_PARAM(ShadowMap, sampler_ShadowMap), float4 shadowCoord, ShadowSamplingData samplingData, half4 shadowParams, bool isPerspectiveProjection = true, float3 hclipPos = 0)
{
    ...
    if(shadowParams.y != 0)
    {
        ...
        // depth = SampleGapFiltered(shadowCoord, samplingData);
    }
    else
#endif
    {
        ...
        // depth = real(SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, shadowCoord.xy).x);
    }
    return BEYOND_SHADOW_FAR(shadowCoord) ? 1.0 : attenuation;
}

half MainLightTerrainRealtimeShadow(float4 shadowCoord, float3 hclipPos)
{
        ...
        return SampleTerrainShadowmap(TEXTURE2D_ARGS(_MainLightSelfShadowmapTexture, sampler_MainLightSelfShadowmapTexture), shadowCoord, shadowSamplingData, shadowParams, false, hclipPos);
    #endif
}


real SampleGapFiltered(float4 shadowCoord, ShadowSamplingData samplingData)
{
//     real gap;
...
//     real4 gap4;
//     float offset = 0.01;
//     gap4.x = real(SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, shadowCoord.xy + offset * (-1,1)).r);
//     gap4.y = real(SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, shadowCoord.xy + offset * (1,-1)).r);
//     gap4.z = real(SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, shadowCoord.xy + offset * (-1,-1)).r);
//     gap4.w = real(SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, shadowCoord.xy + offset * (1,1)).r);
//     gap = dot(gap4, real(0.25));

// #else
//     float fetchesWeights[9];
//     float2 fetchesUV[9];
//     SampleShadow_ComputeSamples_Tent_5x5(samplingData._DMDistanceTextureSize, shadowCoord.xy, fetchesWeights, fetchesUV);

//     gap = fetchesWeights[0] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[0].xy).r;
//     gap += fetchesWeights[1] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[1].xy).r;
//     gap += fetchesWeights[2] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[2].xy).r;
//     gap += fetchesWeights[3] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[3].xy).r;
//     gap += fetchesWeights[4] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[4].xy).r;
//     gap += fetchesWeights[5] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[5].xy).r;
//     gap += fetchesWeights[6] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[6].xy).r;
//     gap += fetchesWeights[7] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[7].xy).r;
//     gap += fetchesWeights[8] * SAMPLE_TEXTURE2D(_DMDistanceTexture, sampler_DMDistanceTexture, fetchesUV[8].xy).r;
// #endif
//     return gap;
    return 1;
}
...
half MainLightTerrainShadow(float4 shadowCoord, float3 positionWS, half4 shadowMask, half4 occlusionProbeChannels, float3 hclipPos)
{
    half realtimeShadow = MainLightTerrainRealtimeShadow(shadowCoord, hclipPos);
