...
namespace ET
{
    public static class SyncData_UnitBaseInfoSystem
    {
        public class AwakeSystem: AwakeSystem<SyncData_UnitBaseInfo>
        {
            protected override void Awake(SyncData_UnitBaseInfo self)
            {
            }
        }

        [ObjectSystem]
        public class SyncData_UnitBaseInfoDestroySystem: DestroySystem<SyncData_UnitBaseInfo>
        {
            protected override void Destroy(SyncData_UnitBaseInfo self)
            {
                self.unitId.Clear();
                self.unitCfgId.Clear();
                self.level.Clear();
                self.unitType.Clear();
                self.posX.Clear();
                self.posY.Clear();
        ...
        public static void Init(this SyncData_UnitBaseInfo self, HashSet<Unit> list)
        {
            self.unitId.Clear();
            self.unitCfgId.Clear();
            self.level.Clear();
            ...
            foreach (var unit in list)
            {
                ...
                self.unitCfgId.Add(unit.CfgId);
                self.level.Add(unit.level);
                self.unitType.Add((int)unit.Type);
        ...
        public static void DealByBytes(this SyncData_UnitBaseInfo self, UnitComponent unitComponent)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                unit.CfgId = self.unitCfgId[i];
                unit.level = self.level[i];
                unit.Type = (UnitType)self.unitType[i];
        ...
        public static float3 GetPos(this SyncData_UnitBaseInfo self, int i)
        {
            return new float3(self.posX[i] * 0.0001f, self.posY[i] * 0.0001f, self.posZ[i] * 0.0001f);
        }

        public static quaternion GetRotation(this SyncData_UnitBaseInfo self, int i)
        {
