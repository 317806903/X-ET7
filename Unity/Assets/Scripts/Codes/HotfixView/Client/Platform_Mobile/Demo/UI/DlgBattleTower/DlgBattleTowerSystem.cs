...
namespace ET.Client
{
    ...
    public static class DlgBattleTowerSystem
    {
        public static void RegisterUIEvent(this DlgBattleTower self)
        {
            ...
            {
                self.View.EGDebugRootRectTransform.SetVisible(true);
                self.View.E_ForceGameEndWhenDebugButton.AddListenerAsync(self.ForceGameEndWhenDebug);
                self.View.E_ForceNextWaveWhenDebugButton.AddListenerAsync(self.ForceNextWaveWhenDebug);
            ...
            {
                self.View.EGDebugRootRectTransform.SetVisible(false);
            }
        }
        ...
        public static async ETTask ShowWindow(this DlgBattleTower self, ShowWindowData contextData = null)
        {
            ...
            await self.ShowGameInstructions();
            await self.PreLoadRes();
        }

        public static void ShowOrHide(this DlgBattleTower self, bool isShow)
        {
        ...
        {
            GamePlayBattleLevelCfg gamePlayBattleLevelCfg = self.GetGamePlayTowerDefense().GetGamePlay().GetGamePlayBattleConfig();
            ...
            string battleCfgId = gamePlayBattleLevelCfg.Id;
            ...
            if (bNeedBattleGuide)
            {
                ...
                await ET.Client.UIGuideHelper.DoUIGuide(self.DomainScene(), battleGuideConfigFileName, (int)ET.Client.GuidePriority.BattleGuide, startIndex,async (scene) =>
                {
                    ...
                    playerOtherInfoComponent.SetBattleGuideFinished(battleCfgId, battleGuideConfigFileName);
        ...
        public static async ETTask ShowGameInstructions(this DlgBattleTower self)
        {
            bool isShowGameInstructions = false;
            if (self.initTime > TimeHelper.ServerNow() - 4000)
            {
                GamePlayTowerDefenseComponent gamePlayTowerDefenseComponent = self.GetGamePlayTowerDefense();
                if (gamePlayTowerDefenseComponent.model.GameInstructions_l10n_key.IsNullOrEmpty())
                {
                    isShowGameInstructions = false;
                }
                else if (gamePlayTowerDefenseComponent.initGameTime > TimeHelper.ServerNow() - 20000)
                {
                    isShowGameInstructions = true;
                }
            }
            if (isShowGameInstructions)
            {
                GamePlayTowerDefenseComponent gamePlayTowerDefenseComponent = self.GetGamePlayTowerDefense();
                self.View.EG_GameInstructionsRectTransform.SetVisible(true);
                self.View.E_GameInstructionsTextTextMeshProUGUI.SetText(gamePlayTowerDefenseComponent.model.GameInstructions);
                self.View.EGRootContentRectTransform.SetVisible(false);
                await TimerComponent.Instance.WaitAsync(1500);
                if (self.IsDisposed)
                {
                    return;
                }
                self.View.EG_GameInstructionsRectTransform.SetVisible(false);
                self.View.EGRootContentRectTransform.SetVisible(true);
            }
            else
            {
                self.View.EG_GameInstructionsRectTransform.SetVisible(false);
        ...
        public static async ETTask PreLoadRes(this DlgBattleTower self)
        {
            await ET.Client.PreLoadResHelper.PreLoadRes_BattleCard(self.DomainScene());

            GamePlayTowerDefenseComponent gamePlayTowerDefenseComponent = self.GetGamePlayTowerDefense();
            await ET.Client.PreLoadResHelper.PreLoadRes_TowerDefenseMonster(self.DomainScene(), gamePlayTowerDefenseComponent.model.Id);
        }
        ...
        public static void SetStep(this DlgBattleTower self)
        {
            ...
            ClientEventType.NoticeGamePlayTowerDefenseStatusWhenClient _NoticeGamePlayTowerDefenseStatusWhenClient = new()
            {
            ...
            if (newGamePlayTowerDefenseStatus == GamePlayTowerDefenseStatus.WaitMeshFinished)
            {
                ...
                EventSystem.Instance.Publish(self.DomainScene(), new ClientEventType.NoticeEventLogging() { eventName = "MeshLoadingStarted"});
                EventSystem.Instance.Publish(self.DomainScene(), new ClientEventType.NoticeEventLoggingStart() { eventName = "MeshLoadingEnded"});
            ...
            else if (newGamePlayTowerDefenseStatus == GamePlayTowerDefenseStatus.PutMonsterPoint)
            {
                ...
                bool isPutHomePlayer = false;
                PutHomeComponent putHomeComponent = gamePlayTowerDefenseComponent.GetComponent<PutHomeComponent>();
                if (putHomeComponent != null)
                {
                    isPutHomePlayer = putHomeComponent.ChkIsPutHomePlayer(myPlayerId);
                    if (isPutHomePlayer)
                    {
                ...
                bool isShowPutMonsterCall = true;
                ...
                if (gamePlayTowerDefenseComponent.isTowerDefenseTeamOne)
                {
                    if (isPutHomePlayer)
                    {
                        isShowPutMonsterCall = true;
                    }
                    else
                    {
                        isShowPutMonsterCall = false;
                    }
                }
                else
                {
                    if (putMonsterCallComponent != null && putMonsterCallComponent.MonsterCallUnitId != null &&
                        putMonsterCallComponent.MonsterCallUnitId.ContainsKey(myPlayerId))
                    {
                        isShowPutMonsterCall = false;
                    }
                    else
                ...
                if (isShowPutMonsterCall == false)
                {
                    self.View.EButton_PutMonsterPointButton.gameObject.SetActive(false);

                    string monsterCallCount = "";
                    if (putMonsterCallComponent != null)
                    {
                        monsterCallCount = putMonsterCallComponent.GetMonsterCallCount();
                    }
                    ET.EventTriggerListener.Get(self.View.EButton_PutMonsterPointButton.gameObject).onDown.RemoveAllListeners();
                    string txt = LocalizeComponent.Instance.GetTextValue("TextCode_Key_Battle_PutMonsterPoint1", monsterCallCount);
                    self.ShowPutTipMsg(txt);
                }
                else
                {
            ...
            else if (newGamePlayTowerDefenseStatus == GamePlayTowerDefenseStatus.RestTime)
            {
                ...
                if (self.gamePlayTowerDefenseStatus != newGamePlayTowerDefenseStatus)
                {
                    ...
                    EventSystem.Instance.Publish(self.DomainScene(), new ClientEventType.NoticeEventLoggingStart() { eventName = "ShoppingEnded"});
                }
            }
            else if (newGamePlayTowerDefenseStatus == GamePlayTowerDefenseStatus.InTheBattle)
            {
                ...
                if (self.gamePlayTowerDefenseStatus != newGamePlayTowerDefenseStatus)
                {
                    ...
                    EventSystem.Instance.Publish(self.DomainScene(), new ClientEventType.NoticeEventLoggingStart() { eventName = "WaveEnded"});
                }
            }
            ...
            else if (newGamePlayTowerDefenseStatus == GamePlayTowerDefenseStatus.RecoverSuccess)
            {
                UIAudioManagerHelper.PlayUIAudio(self.DomainScene(), SoundEffectType.Revive);
            }
            else
            {
            }
        ...
        public static void RefreshCoinAnimation(this DlgBattleTower self)
        {
            // 动画时间
            float duration = 0.5f;
        ...
        public static void ChkRefreshBuyTowerPannel(this DlgBattleTower self)
        {
            ...
            {
                bool isNeedRefreshCell = false;
                if (gamePlayTowerDefenseComponent.model.RefreshBuyTowerCost > self.GetMyGold())
                {
                    if (self.View.ELabel_RefreshTextMeshProUGUI.color != Color.red)
                    {
                        isNeedRefreshCell = true;
                        self.View.ELabel_RefreshTextMeshProUGUI.color = Color.red;
                    }
                }
                else
                {
                    if (self.View.ELabel_RefreshTextMeshProUGUI.color != Color.white)
                    {
                        isNeedRefreshCell = true;
                        self.View.ELabel_RefreshTextMeshProUGUI.color = Color.white;
                    }
                }
                if (isNeedRefreshCell)
                {
            ...
            {
                MonsterWaveCallComponent monsterWaveCallComponent = gamePlayTowerDefenseComponent.GetComponent<MonsterWaveCallComponent>();
                if (monsterWaveCallComponent.isNoRest)
                {
                    bool isNeedRefreshCell = false;
                    if (gamePlayTowerDefenseComponent.model.RefreshBuyTowerCost > self.GetMyGold())
                    {
        ...
        public static void ResetScrollRectMoveWhenGuide(this DlgBattleTower self)
        {
        ...
        public static void SetCurLeftTimeInfo(this DlgBattleTower self)
        {
            ...
            else if (gamePlayTowerDefenseStatus == GamePlayTowerDefenseStatus.InTheBattle)
            {
                ...
                if (monsterWaveCallComponent.isNoRest == false)
                {
        ...
        public static void ShowCurLeftTimeInfo(this DlgBattleTower self)
        {
            ...
            if (self.leftTimeShow == 0 || self.leftTimeShow != leftTimeShow)
            {
                self.leftTimeShow = leftTimeShow;
        ...
        public static void RefreshPlayerTowerAnimation(this DlgBattleTower self)
        {
            // 可以添加额外的视觉效果，例如缩放动画
        ...
        public static List<(string, int)> GetTowerBuyList(this DlgBattleTower self)
        {
            GamePlayTowerDefenseComponent gamePlayTowerDefenseComponent = self.GetGamePlayTowerDefense();
        ...
        public static int GetTowerBuyCost(this DlgBattleTower self, int index)
        {
            GamePlayTowerDefenseComponent gamePlayTowerDefenseComponent = self.GetGamePlayTowerDefense();
            PlayerOwnerTowersComponent playerOwnerTowersComponent = gamePlayTowerDefenseComponent.GetComponent<PlayerOwnerTowersComponent>();
            long playerId = PlayerStatusHelper.GetMyPlayerId(self.DomainScene());
        ...
        public static List<string> GetOwnerTowerList(this DlgBattleTower self)
        {
            ...
            {
                if (ET.ItemHelper.ChkIsSkill(x) && ET.ItemHelper.ChkIsSkill(y))
                {
                    return ItemCfgCategory.Instance.Get(y).ShowPriority.CompareTo(ItemCfgCategory.Instance.Get(x).ShowPriority);
                }
                else if (ET.ItemHelper.ChkIsSkill(x))
                {
                    return 1;
                }
                else if (ET.ItemHelper.ChkIsSkill(y))
                {
                    return -1;
                }
                else
        ...
        public static async ETTask NotTowerBuyShowWhenBattle(this DlgBattleTower self)
        {
            ...
            {
                self.View.EG_BuyNodeRectTransform.gameObject.SetActive(true);
                self.View.EButton_ReadyWhenRestTimeButton.gameObject.SetActive(false);
            }
        }
        ...
        public static void OnSelectTower(this DlgBattleTower self, string towerCfgId)
        {
            ...
            self.View.E_CoinsAndTowersImage.SetVisible(false);
            self.View.ELabel_LeftMonsterWave_boxImage.SetVisible(false);
        ...
        public static void AddTowerItemRefreshListener(this DlgBattleTower self, Transform transform, int index)
        {
            ...
            ET.EventTriggerListener.Get(itemTower.GetActionButton()).onPress.AddListener((go, xx) =>
            {
                ...
                {
                    Log.Error($"skillCfgId[{itemCfgId}] press");
                }
            });
            ...
            ET.EventTriggerListener.Get(itemTower.GetActionButton()).onClick.AddListener((go, xx) =>
            {
                if (ET.ItemHelper.ChkIsTower(itemCfgId))
                {
                    if (ET.Client.UIGuideHelper.ChkIsUIGuideing(self.DomainScene(), true) == false)
                    {
                        ...
                        {
                            playerId = myPlayerId,
                            towerCfgId = itemCfgId,
                        });
                    }
                }
                else if (ET.ItemHelper.ChkIsSkill(itemCfgId))
                {
                    Log.Error($"skillCfgId[{itemCfgId}] click");
                }
            });
        ...
        public static void AddTowerBuyListener(this DlgBattleTower self, Transform transform, int index)
        {
            ...
            List<(string towerCfgId, int num)> list = self.GetTowerBuyList();
            List<bool> listBought = self.GetTowerBoughtsList();

            string towerCfgId = list[index].towerCfgId;
            int itemNum = list[index].num;
            string itemCfgId = towerCfgId;

            itemTowerBuy.Init(itemCfgId, false, itemNum).Coroutine();
            ...
            (bool bRet, string tutorialCfgId) = ET.ItemHelper.ChkIsNeedShowTutorialInBattle(towerCfgId);
            if (bRet)
            {
        ...
        public static async ETTask ForceNextWaveWhenDebug(this DlgBattleTower self)
        {
            UIAudioManagerHelper.PlayUIAudio(self.DomainScene(),SoundEffectType.Back);

            string msgTxt = LocalizeComponent.Instance.GetTextValue("是否快速跳过当前波次(需要已放置大本营)");
            ET.Client.UIManagerHelper.ShowConfirm(self.DomainScene(), msgTxt, () =>
            {
                ET.Client.GamePlayHelper.SendForceNextWaveWhenDebug(self.DomainScene()).Coroutine();
            }, null);
        }
        ...
        public static void ResetUIBase(this DlgBattleTower self)
        {
            self.ShowOrHide(true);
            self.View.E_TipNodeImage.SetVisible(false);
            self.View.EG_BuyNodeRectTransform.SetVisible(true);
            //self.View.E_QuitBattleButton.SetVisible(true);
            self.View.E_CoinsAndTowersImage.SetVisible(true);
            self.View.ELabel_LeftMonsterWave_boxImage.SetVisible(true);
        ...
        public static async ETTask OnResetHeadQuarter(this DlgBattleTower self)
        {
            await ET.Client.GamePlayTowerDefenseHelper.SendResetHome(self.ClientScene());
        }

        public static async ETTask OnSelectHeadQuarter(this DlgBattleTower self)
        {
        ...
        public static async ETTask OnSelectMonsterCall(this DlgBattleTower self)
        {
            await TimerComponent.Instance.WaitFrameAsync();
            await TimerComponent.Instance.WaitFrameAsync();
            self.View.E_PutHomeAndMonsterPointImage.gameObject.SetActive(false);
