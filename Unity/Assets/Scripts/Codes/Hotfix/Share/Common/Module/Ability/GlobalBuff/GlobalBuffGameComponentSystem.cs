...
namespace ET.Ability
{
    ...
    public static class GlobalBuffGameComponentSystem
    {
        ...
        public static async ETTask<GlobalBuffGameObj> AddGlobalBuff(this GlobalBuffGameComponent self, TeamFlagType teamFlagType, long casterPlayerId, string homeTeamGlobalBuffCfgId)
        {
            (bool isExist, GlobalBuffGameObj globalBuffGameObj) = self.ChkExist(teamFlagType, casterPlayerId, homeTeamGlobalBuffCfgId);
            if (isExist)
            {
                if (globalBuffGameObj.casterPlayerId == (long)ET.PlayerId.PlayerNone)
                {
            ...
            {
                bool IsEnabled = self.ChkIsEnabledByTagGroup(homeTeamGlobalBuffCfgId);

                globalBuffGameObj = self.AddChild<GlobalBuffGameObj>();
                globalBuffGameObj.isEnabled = IsEnabled;
                await globalBuffGameObj.Init(teamFlagType, casterPlayerId, homeTeamGlobalBuffCfgId);
        ...
        public static (bool, GlobalBuffGameObj) ChkExist(this GlobalBuffGameComponent self, TeamFlagType teamFlagType, long casterPlayerId, string homeTeamGlobalBuffCfgId)
        {
            ...
            foreach (var obj in self.Children.Values)
            {
                ...
                if (globalBuffGameObj.CfgId != homeTeamGlobalBuffCfgId)
                {
                    continue;
                }
