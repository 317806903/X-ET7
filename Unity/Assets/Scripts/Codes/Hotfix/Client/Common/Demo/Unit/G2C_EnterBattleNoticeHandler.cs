...
namespace ET.Client
{
	...
	public class G2C_EnterBattleNoticeHandler : AMHandler<G2C_EnterBattleNotice>
	{
		protected override async ETTask Run(Session session, G2C_EnterBattleNotice message)
		{
			...
			EventSystem.Instance.Publish(clientScene, new ClientEventType.NoticeUIShowCommonLoading());

			// 等待场景切换完成
			await clientScene.GetComponent<ObjectWait>().Wait<Wait_SceneChangeFinish>();

			Log.Debug("G2C_EnterBattleNotice 22");
			EventSystem.Instance.Publish(clientScene, new ClientEventType.EnterMapFinish());

			EventSystem.Instance.Publish(clientScene, new ClientEventType.NoticeUIHideCommonLoading());
