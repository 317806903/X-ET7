...
namespace ET.Client
{
    public static class PathLineRendererComponentSystem
    {
        ...
        public static async ETTask ShowPath(this PathLineRendererComponent self, TeamFlagType homeTeamFlagType, long monsterCallUnitId, bool canArrive, List<float3> points)
        {
            ...
            else
            {
                if (self.lineRendererRoot == null)
                {
                    ...
                    PathLineRendererGo.transform.localScale = Vector3.one * clientResScale;
            ...
            {
                float3 midPos = RecastHelper.GetLengthMidPosByList(points);
                midPos = ET.Ability.UnitHelper.TranServerPos2ClientPos(self.DomainScene(), midPos);
                self.lineRendererMidPos[key] = midPos;
                ...
                lineRendererHeight *= clientResScale;

                Vector3[] linePoints = new Vector3[points.Count];
                for (int i = 0; i < points.Count; i++)
                {
                    float3 pos = points[i];
                    pos = ET.Ability.UnitHelper.TranServerPos2ClientPos(self.DomainScene(), pos);
                    linePoints[i] = new Vector3(0, lineRendererHeight, 0) + (Vector3)pos;
                }
        ...
        {
            float clientResScale = ET.Ability.UnitHelper.GetClientGameResScale(self.DomainScene());
            int hitCount = Physics.SphereCastNonAlloc(pos + new float3(0, 5, 0) * clientResScale, radius + 0.5f * clientResScale, Vector3.down, self.results);
