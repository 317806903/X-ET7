...
namespace ET.Client
{
	...
	{
		public static UIGuideComponent GetUIGuideComponent(Scene scene)
		{
			...
			return UIGuideComponent.Instance;
		}
		...
		{
			await GetUIGuideComponent(scene).DoUIGuideByName(fileName, priority, startIndex, finished, stepFinished);
		}
		...
		{
			await GetUIGuideComponent(scene).DoUIGuide(guideFileName, priority, _UIGuidePathList, startIndex, finished, stepFinished);
		}

		public static void ClearUIGuide()
		{
			if (UIGuideComponent.Instance == null)
			{
				return;
