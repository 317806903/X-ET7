...
namespace ET.Server
{
    [ActionFromHttpHandler(ActionFromHttpStatus.SetItemNum)]
    public class ActionFromHttpHandler_SetItemNum: IActionFromHttpHandler
    {
        ...
        public override async ETTask<(bool bRet, string msg)> Run(Scene scene, Dictionary<string, string> paramDic)
        {
            ...
            foreach (long playerId in playerList)
            {
                foreach (var item in this.itemCfgList)
                {
                    ...
                    int num = item.Value;
                    await PlayerCacheHelper.SetItemNum(scene, playerId, itemCfgId, num);
                }
            }
            return (true, "");
        }
