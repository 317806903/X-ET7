...
namespace DynamicAtlas
{
    public class DynamicAtlas
    {
        ...
        public Dictionary<string, SaveTextureData> GetUsingTexture()
        {
            return this.m_UsingTexture;
        }
        ...
        public void GetTeture(string name, Texture2D texture, OnCallBackTexRect callback)
        {
...
#if UNITY_EDITOR
            saveTextureData.assetPath = UnityEditor.AssetDatabase.GetAssetPath(texture2D);
#endif
            ...
            for (int i = page.freeAreasList.Count - 1; i >= 0; i--)
            {
                ...
                if (!(x >= area.right || right <= area.x || y >= area.top || top <= area.y))
                {
                    if (targetWithPadding == null)
                        targetWithPadding =
                            DynamicAtlasMgr.Instance.AllocateIntegerRectangle(target.x, target.y, target.width + m_Padding,
                                target.height + m_Padding);
    ...
    public class DynamicAtlasPage
    {
        ...
        public DynamicAtlasPage(int index, int width, int height, Color32[] tempColor)
        {
            ...
            m_Texture = new Texture2D(width, height, DynamicAtlasConfig.kTextureFormat, false, false);
        ...
        public static bool ChkTextureCanDynamic(Texture2D srcTex)
        {
            bool isUseGetSetPixels = false;
            bool isUseGraphicsCopyTexture = false;
            //可以把一张贴图画到另一张贴图上
            if (SystemInfo.copyTextureSupport == UnityEngine.Rendering.CopyTextureSupport.None)
            {
                isUseGetSetPixels = true;
                isUseGraphicsCopyTexture = false;
            }
            else
            {
                if (srcTex.graphicsFormat == GraphicsFormat.RGB_ETC2_SRGB ||
                    srcTex.graphicsFormat == GraphicsFormat.RGB_A1_ETC2_SRGB ||
                    srcTex.graphicsFormat == GraphicsFormat.RGBA_ETC2_SRGB)
                {
                    int i = 5;
                }

                if (srcTex.format == TextureFormat.RGBA32)
                {
                    isUseGetSetPixels = false;
                    isUseGraphicsCopyTexture = true;
                }
                else
                {
                    isUseGetSetPixels = true;
        ...
        public void AddTexture(int posX, int posY, Texture2D srcTex)
        {
            try
            {
                ...
                if (isUseGetSetPixels)
                {
                    ...
                    {
                        //High GC allocs here
                        var pixelBuffer = srcTex.GetPixels(0, 0, srcTex.width, srcTex.height);
                        this.m_Texture.SetPixels(posX, posY, srcTex.width, srcTex.height, pixelBuffer);
                        this.m_Texture.Apply(false);
                    }
                    else
                    {
                        Debug.LogError($"srcTex[{srcTex.name}].isReadable == false");
                    }
                }
        ...
        public int GetFreeAreaIndex(int width, int height, int padding)
        {
            ...
            for (int i = m_FreeAreasList.Count - 1; i >= 0; i--)
            {
                ...
                // if (free.x < width || free.y < height)
                {
                    ...
                    {
                        index = i;
                        if ((paddedWidth == free.width && free.width <= free.height && free.right < m_Width) ||
                            (paddedHeight == free.height && free.height <= free.width)) //如果这个区域正好可以放得下
                            break;
                        best = free;
                    }
                    else
                    {
                        ...
                        {
                            index = i;
                            if ((width == free.width && free.width <= free.height && free.right < m_Width) ||
