...
namespace ET.Client
{
    ...
    {
        public static async ETTask SendPutHome(Scene scene, string unitCfgId, float3 position, float3 forward)
        {
            ...
            else
            {
                ...
                EventSystem.Instance.Publish(scene, new ClientEventType.NoticeEventLoggingStart()
                {
        ...
        public static async ETTask<(float3, List<float3>)> SendGetMonsterCall2HeadQuarterPath(Scene scene, TeamFlagType homeTeamFlagType, float3 position)
        {
            position = ET.Ability.UnitHelper.TranClientPos2ServerPos(scene, position);
        ...
        public static async ETTask<M2C_TryMoveUnitAndGetAllMonsterCall2HeadQuarterPath> SendTryMoveUnitAndGetAllMonsterCall2HeadQuarterPath(
            Scene scene, long unitId, string unitCfgId, float3 position)
        {
            position = ET.Ability.UnitHelper.TranClientPos2ServerPos(scene, position);
        ...
        public static async ETTask<(bool, string)> SendPutMonsterCall(Scene scene, string unitCfgId, float3 position)
        {
            position = ET.Ability.UnitHelper.TranClientPos2ServerPos(scene, position);
        ...
        public static async ETTask SendCallOwnTower(Scene scene, string towerUnitCfgId, float3 position, float3 forward)
        {
            position = ET.Ability.UnitHelper.TranClientPos2ServerPos(scene, position);
            forward = ET.Ability.UnitHelper.TranClientForward2ServerForward(forward);
            ...
            {
                EventSystem.Instance.Publish(scene, new ClientEventType.NoticeGuideConditionStatus()
                {
        ...
        public static async ETTask SendUpgradeItemUnit(Scene scene, long itemUnitId, string itemCfgId, int nextLevel, string itemGiftCfgId)
        {
            C2M_UpgradeItemUnit _C2M_UpgradeItemUnit = new()
            {
                ItemUnitId = itemUnitId,
                NextLevel = nextLevel,
                ItemGiftCfgId = itemGiftCfgId,
            };
            M2C_UpgradeItemUnit _M2C_UpgradeItemUnit =
                await SessionHelper.GetSession(scene).Call(_C2M_UpgradeItemUnit) as M2C_UpgradeItemUnit;
            if (_M2C_UpgradeItemUnit.Error != ErrorCode.ERR_Success)
            {
                EventSystem.Instance.Publish(scene, new ClientEventType.NoticeUITip()
                {
                    tipMsg = _M2C_UpgradeItemUnit.Message,
                });
        ...
        public static async ETTask SendMovePlayerTower(Scene scene, long towerUnitId, float3 position, float3 forward)
        {
            position = ET.Ability.UnitHelper.TranClientPos2ServerPos(scene, position);
            forward = ET.Ability.UnitHelper.TranClientForward2ServerForward(forward);
        ...
        public static async Task<(long, float3)> GetNearestNavmeshPos(Scene scene, float3 position)
        {
            position = ET.Ability.UnitHelper.TranClientPos2ServerPos(scene, position);
            try
            {
