...
public partial class UICodeSpawner
{
    static public void SpawnLoopItemCode(GameObject gameObject)
    {
        ...
        SpawnCodeForScrollLoopItemSystem(gameObject);
        SpawnCodeForScrollLoopItemBehaviour(gameObject);
        SpawnCodeForScrollLoopItemViewSystem(gameObject);
        AssetDatabase.Refresh();
    }

    static void SpawnCodeForScrollLoopItemSystem(GameObject gameObject)
    {
        string strDlgName = $"Scroll_{gameObject.name}";
        string strFilePath = Application.dataPath + $"/Scripts/Codes/HotfixView/Client/{UICodeSpawner.GetPlatform()}/Demo/UI/UIItem/" + strDlgName;
        ...
        strFilePath = Application.dataPath + $"/Scripts/Codes/HotfixView/Client/{UICodeSpawner.GetPlatform()}/Demo/UI/UIItem/" + strDlgName + "/" + strDlgName + "System.cs";
        ...
        strBuilder.AppendFormat("\t\tpublic static void HideItem(this {0} self)\n", strDlgName)
            .AppendLine("\t\t{")
            .AppendLine("\t\t}")
            .AppendLine();
    ...
    static void SpawnCodeForScrollLoopItemViewSystem(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"/Scripts/Codes/HotfixView/Client/{UICodeSpawner.GetPlatform()}/Demo/UIBehaviour/UIItemBehaviour";

        if ( !System.IO.Directory.Exists(strFilePath) )
        {
            System.IO.Directory.CreateDirectory(strFilePath);
        }
        strFilePath     = Application.dataPath + $"/Scripts/Codes/HotfixView/Client/{UICodeSpawner.GetPlatform()}/Demo/UIBehaviour/UIItemBehaviour/" + strDlgName + "ViewSystem.cs";
        StreamWriter sw = new StreamWriter(strFilePath, false, Encoding.UTF8);
        ...
        strBuilder.AppendLine("\t{");
        strBuilder.AppendFormat("\t\tprotected override void Awake( {0} self )",strDlgName);
        strBuilder.AppendLine("\n\t\t{");
        strBuilder.AppendFormat("\t\t\tself.RegisterUIEvent();\r\n");
        strBuilder.AppendLine("\t\t}");
        strBuilder.AppendLine("\t}");
        ...
        strBuilder.AppendLine("\n\t\t{");
        strBuilder.AppendFormat("\t\t\tself.HideItem();\r\n");
    ...
    static void SpawnCodeForScrollLoopItem(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"/Scripts/Codes/ModelView/Client/{UICodeSpawner.GetPlatform()}/Demo/UI/UIItem";

        if ( !System.IO.Directory.Exists(strFilePath) )
        {
            System.IO.Directory.CreateDirectory(strFilePath);
        }
        strFilePath     = Application.dataPath + $"/Scripts/Codes/ModelView/Client/{UICodeSpawner.GetPlatform()}/Demo/UI/UIItem/" + strDlgName + ".cs";
        if (System.IO.File.Exists(strFilePath))
        {
    ...
    static void SpawnCodeForScrollLoopItemBehaviour(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"/Scripts/Codes/ModelView/Client/{UICodeSpawner.GetPlatform()}/Demo/UIBehaviour/UIItemBehaviour";

        if ( !System.IO.Directory.Exists(strFilePath) )
        {
            System.IO.Directory.CreateDirectory(strFilePath);
        }
        strFilePath     = Application.dataPath + $"/Scripts/Codes/ModelView/Client/{UICodeSpawner.GetPlatform()}/Demo/UIBehaviour/UIItemBehaviour/" + strDlgName + "View.cs";
        StreamWriter sw = new StreamWriter(strFilePath, false, Encoding.UTF8);
        ...
        strBuilder.AppendFormat("\tpublic partial class {0} : Entity, IAwake, IDestroy, IUIScrollItem, IUILogic\r\n", strDlgName)
            .AppendLine("\t{");
        ...
        strBuilder.AppendLine("\t\t}\n");
        strBuilder.AppendFormat("\t\tpublic {0} BindTrans(Transform trans)\r\n",strDlgName);
        strBuilder.AppendLine("\t\t{");
