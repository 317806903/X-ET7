...
namespace DynamicAtlas
{
    ...
    public class DynamicImage: MonoBehaviour
    {
        ...
        private void OnPreDoImage()
        {
            ...
            if (this.m_Image.sprite != null) //事先挂载了一张图片
            {
                if (this.ChkTextureCanDynamic(this.m_Image.sprite.texture) == false)
                {
                    return;
                }
        ...
        {
            m_DefaultSprite = this.m_Image.sprite;
            m_SpriteName = this.m_Image.mainTexture.name;
            m_Atlas.SetTexture(this.m_Image.mainTexture, OnGetImageCallBack);
        }
        ...
        private void OnGetImageCallBack(Texture tex, Rect rect)
        {
            ...
            if (m_DefaultSprite != null)
                this.m_Image.sprite = Sprite.Create((Texture2D)tex, spriteRect, m_DefaultSprite.pivot, m_DefaultSprite.pixelsPerUnit, 1,
                    SpriteMeshType.Tight, m_DefaultSprite.border);
            else
            {
                this.m_Image.sprite = Sprite.Create((Texture2D)tex, spriteRect, new Vector2(spriteRect.width * .5f, spriteRect.height * .5f), 100, 1,
