    ...
    public static class NavmeshManagerComponentSystem
    {
        ...
        public class AwakeSystem: AwakeSystem<NavmeshManagerComponent>
        {
            protected override void Awake(NavmeshManagerComponent self)
            {
                ...
                self.recordNavColliderTower = new();
        ...
        public class DestroySystem: DestroySystem<NavmeshManagerComponent>
        {
            protected override void Destroy(NavmeshManagerComponent self)
            {
                ...
                self.recordNavColliderTower.Clear();
            }
        }
        ...
        public static async ETTask<NavmeshComponent> GetUnitFlyNavmesh(this NavmeshManagerComponent self)
        {
            while (!self.isLoadMeshFinished)
            {
                if (self.IsDisposed)
            ...
            NavmeshComponent navmeshComponent = self.unitFlyNavmesh;
            if (navmeshComponent != null)
            {
                return navmeshComponent;
            }
            ...
            navmeshComponent = self.AddChild<NavmeshComponent>();
            await navmeshComponent.CreateCrowd(self.navSample.GetSettings().agentRadius);
            self.unitFlyNavmesh = navmeshComponent;
            return navmeshComponent;
        }
        ...
        public static void ResetSampleSettings(this NavmeshManagerComponent self, Sample sample, float scale)
        {
            ...
            rcNavMeshBuildSettings.tileSize = 16;
        }
        ...
        public static void _InitNavMeshBuilder(this NavmeshManagerComponent self)
        {
            ...
            self.navSample.SetRecastResultsOrg(dynaMesh.RecastResults());
        ...
        public static (bool, float3) _ChkHitMesh(this NavmeshManagerComponent self, float3 rayStartIn, float3 rayEndIn, float segmentCount)
        {
            for (int i = 0; i <= segmentCount; i++)
            {
                ...
                bool bHitNavColliderTower = self.ChkHitNavColliderTower(pos);
                if (bHitNavColliderTower)
                {
                    return (true, pos);
                }
            }
        ...
        public static bool ChkHitMeshOnPoint(this NavmeshManagerComponent self, float3 rayPosIn)
        {
            ...
            foreach (RcBuilderResult recastBuilderResult in self.GetSample().GetRecastResultsOrg())
            {
        ...
        public static NavmeshManagerComponent.NavMeshData GetNavMeshData(this NavmeshManagerComponent self, float3 startPosition)
        {
            ...
            List<long> polys = ListComponent<long>.Create();
            List<long> resultParent = ListComponent<long>.Create();
            List<float> resultCost = ListComponent<float>.Create();
            List<(DtMeshTile, DtPoly)> tilesAndPolygons = ListComponent<(DtMeshTile, DtPoly)>.Create();
            ...
            List<long> polyRefs = ListComponent<long>.Create();
        ...
        public static (bool, float3) _GetMeshHeightfield(this NavmeshManagerComponent self, int indexX, int indexZ, RcHeightfield hf)
        {
            RcVec3f orig = hf.bmin;
            float cs = hf.cs;
            float ch = hf.ch;
            ...
            while (s != null)
            {
                if (s.next == null)
                {
                    float curHeight = orig.Y + s.smax * ch;
                    RcVec3f rcVec3F = new RcVec3f(orig.X + indexX * cs, curHeight, orig.Z + indexZ * cs);
                    return (true, RecastHelper.ToFloat3(rcVec3F));
                }
                s = s.next;
            }
        ...
        public static bool UpdateDynamicMesh(this NavmeshManagerComponent self, bool isNeedUpdateCrowd)
        {
            if (self.dynamicMesh != null)
            {
                if (self.dynamicMesh.Update(Task.Factory))
                {
                    ...
                    if (isNeedUpdateCrowd)
                    {
                        NavmeshComponent player = self.playerNavmesh;
                        if (player != null)
                        {
                            player.OnNavMeshUpdateCrowd();
                        }
                        NavmeshComponent unit = self.unitNavmesh;
                        if (unit != null)
                        {
                            unit.OnNavMeshUpdateCrowd();
                        }
                        NavmeshComponent unitFly = self.unitFlyNavmesh;
                        if (unitFly != null)
                        {
                            unitFly.OnNavMeshUpdateCrowd();
                        }
                    }
        ...
        public static long AddObstacle(this NavmeshManagerComponent self, float3 pos, float radius, float height)
        {
            if (self.dynamicMesh == null)
            {
                return -1;
            ...
            float cellHeight = self.GetSample().GetSettings().cellHeight;
            float agentMaxClimb = self.GetSample().GetSettings().agentMaxClimb;
            height = math.max(height, agentMaxClimb+cellHeight);
            var newPos = RecastHelper.ToRcVec3f(pos) + new RcVec3f(0, math.max(height/2, 0), 0);
            RcVec3f extent = new RcVec3f(radius, height/2, radius);
        ...
        {
            self.UpdateDynamicMesh(true);
        }

        public static void UpdateRecordNavColliderTower(this NavmeshManagerComponent self, long unitId, float3 pos, float radius, float height)
        {
            self.recordNavColliderTower[unitId] = (pos, radius, height);
        }

        public static void RemoveRecordNavColliderTower(this NavmeshManagerComponent self, long unitId)
        {
            self.recordNavColliderTower.Remove(unitId);
        }

        public static bool ChkHitNavColliderTower(this NavmeshManagerComponent self, float3 posChk)
        {
            foreach (var item in self.recordNavColliderTower)
            {
                (float3 pos, float radius, float height) = item.Value;
                if (posChk.y > pos.y + height || posChk.y < pos.y)
                {
                    continue;
                }

                if (math.abs(pos.x - posChk.x) > radius || math.abs(pos.z - posChk.z) > radius)
                {
                    continue;
                }
        ...
        public static List<float3> GetRandomPointFromMesh(this NavmeshManagerComponent self, float pointDis)
        {
            ListComponent<float3> randomPointList = ListComponent<float3>.Create();
            foreach (RcBuilderResult recastBuilderResult in self.GetSample().GetRecastResultsOrg())
            {
                RcHeightfield rcHeightfield = recastBuilderResult.SolidHeightfiled;
                try
                {
                    RcVec3f orig = rcHeightfield.bmin;
                    float cs = rcHeightfield.cs;
                    float ch = rcHeightfield.ch;

                    int w = rcHeightfield.width;
                    int h = rcHeightfield.height;

                    int count = (int)(pointDis / cs);
                    for (int i = 0; i < w/count; i++)
                    {
                        for (int j = 0; j < h/count; j++)
                        {
                            (bool bRet, float3 pos) = self._GetMeshHeightfield(i * count, j * count, rcHeightfield);
                            if (bRet)
                            {
                                randomPointList.Add(pos);
                            }
                        }
                    }
                }
