...
namespace ET.Server
{
    ...
    public static class PlayerMailComponentSystem
    {
        ...
        /// <returns></returns>
        public static async ETTask<bool> Refresh(this PlayerMailComponent self)
        {
            ...
            if (bRet)
            {
                PlayerBaseInfoComponent playerBaseInfoComponent =
                    await PlayerCacheLocalHelper.GetPlayerModel(self.DomainScene(), self.GetPlayerId(), PlayerModelType.BaseInfo) as PlayerBaseInfoComponent;

                foreach (byte[] bytes in componentBytes)
                {
                    MailInfoComponent mailInfoComponent = MongoHelper.Deserialize<MailInfoComponent>(bytes);
                    mailInfoComponent.mailTitle = mailInfoComponent.mailTitle.Replace("{Name}", playerBaseInfoComponent.PlayerName);
                    mailInfoComponent.mailContent = mailInfoComponent.mailContent.Replace("{Name}", playerBaseInfoComponent.PlayerName);
