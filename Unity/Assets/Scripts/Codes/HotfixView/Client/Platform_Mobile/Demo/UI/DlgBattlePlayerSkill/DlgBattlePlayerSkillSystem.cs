...
namespace ET.Client
{
	...
	public static class DlgBattlePlayerSkillSystem
	{
		...
		public static void ShowOrHide(this DlgBattlePlayerSkill self, bool isShow)
		{
		...
		{
			Unit myUnit = self.GetPlayerUnit();
			if (myUnit == null)
			{
				return;
			}
        ...
        {
	        ET.Ability.SkillObj skillObj = ET.Ability.SkillHelper.GetSkillObj(self.GetPlayerUnit(), skillCfgId);
        ...
        public static void AddSkillItemRefreshListener(this DlgBattlePlayerSkill self, Transform transform, int index)
        {
            ...
            List<ET.Ability.SkillObj> skillList = ET.Ability.SkillHelper.GetManualSkillList(self.GetPlayerUnit());
        ...
        public static Unit GetPlayerUnit(this DlgBattlePlayerSkill self)
        {
        ...
        public static void CastSkillWhenTrig(this DlgBattlePlayerSkill self, string skillCfgId)
        {
                ...
                manualSkillCfg.SkillSelectAction_Ref.ActionCallParam is ActionCallShow_Drag_OtherUnit)
            {
                ...
                selectHandle = ET.Client.UnitViewHelper.CreateSelectHandle(self.GetPlayerUnit(), isResetPos, resetPos, isResetForward, resetForward, selectObjectCfg, ref actionContext);
            }
            else
            {
        ...
        {
	        (bool bRet, string msg) = ET.Ability.SkillHelper.ChkCanUseSkill(self.GetPlayerUnit(), skillCfgId);
	        if (bRet == false)
	        {
	        ...
	        bRet = await SkillHelper.CastSkill(self.DomainScene(), false, skillCfgId, self.unitId, float3.zero, float3.zero, selectHandle);
