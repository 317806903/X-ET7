...
namespace ET
{
    ...
    public class BuildEditor: EditorWindow
    {
        private Dictionary<ServerEnum, string> serverAddressList = new()
        {
            ...
            { ServerEnum.Release_108, "192.168.10.108"},
            { ServerEnum.Release_OutNet_Arcade, "8.134.207.186"},
            { ServerEnum.Release_OutNet_CN, "8.134.176.17"},
            { ServerEnum.Release_OutNet_CN_Demo, "8.134.176.17"},
            { ServerEnum.Release_OutNet_Quest, "8.134.176.17"},
        ...
        private Dictionary<ServerEnum, string> hotfixAddressList = new()
        {
            ...
            { ServerEnum.Release_108, "http://192.168.10.108"},
            ...
            { ServerEnum.Release_OutNet_Quest, "https://omelette.oss-cn-beijing.aliyuncs.com/dev/DeepMirrorARGame_Quest"},
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("ResetLocalEditor"))
                {
                    ...
                    if (this.globalConfig.clientResScaleType != ClientResScaleType.ResScaleAndCameraOrg)
                    {
                        this.globalConfig.clientResScaleType = ClientResScaleType.ResScaleAndCameraOrg;
                        EditorUtility.SetDirty(this.globalConfig);
                        AssetDatabase.SaveAssets();
                    }
                    ...
                    if (this.resConfig.Channel != "10000")
                    {
                        this.resConfig.Channel = "10000";
                        EditorUtility.SetDirty(this.resConfig);
                        AssetDatabase.SaveAssets();
                    }
            ...
            GUILayout.Space(5);
            EditorGUI.BeginChangeCheck();
            ClientResScaleType clientResScaleType = (ClientResScaleType)EditorGUILayout.EnumPopup("ClientResScaleType: ",this.globalConfig.clientResScaleType);
            if (EditorGUI.EndChangeCheck())
            {
                if (clientResScaleType != this.globalConfig.clientResScaleType)
                {
                    this.globalConfig.clientResScaleType = clientResScaleType;
                    EditorUtility.SetDirty(this.globalConfig);
                    AssetDatabase.SaveAssets();
                }
            }
            else
            {
                if (clientResScaleType != this.globalConfig.clientResScaleType)
                {
                    clientResScaleType = this.globalConfig.clientResScaleType;
                }
            }
            GUILayout.Space(5);
            EditorGUI.BeginChangeCheck();
