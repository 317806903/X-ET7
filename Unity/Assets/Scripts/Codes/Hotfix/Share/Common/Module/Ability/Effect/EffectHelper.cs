        ...
        public static async ETTask AddEffectWhenSelectUnits(Unit unit, ActionCfg_EffectCreate actionCfgCreateEffect, SelectHandle selectHandle, ActionContext actionContext)
        {
            ...
            else
            {
                ...
                foreach (var unitId in selectHandle.unitIds)
                {
                    ...
                    if (ET.Ability.UnitHelper.ChkIsFly(unitEffect))
                    {
                        float gameResScale = ET.Ability.UnitHelper.GetGameResScale(unitEffect.DomainScene());
                        position += new float3(0, GlobalSettingCfgCategory.Instance.FlyHeight * gameResScale, 0);
                    }
