...
public class ShootTextProManager : MonoBehaviour
{
    ...
    private RectTransform shootTextCanvas = null;
    public RectTransform ShootTextCanvas
    {
        get
        {
    ...
    private Camera uiTextCamera = null;
    public Camera UITextCamera
    {
        get
        {
            return uiTextCamera;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < handleShootTextGroup.Count; i++)
        {
            ...
            if (this.UITextCamera != this.ShootTextCamera)
            {
            ...
            {
                shootTextComponent.transform.forward = ShootTextCamera.transform.forward;

                shootTextCreatPosition += shootTextComponent.transform.right * (float)(shootTextComponent.initializedHorizontalPositionOffset + shootTextComponent.xMoveOffeset);
                shootTextCreatPosition += shootTextComponent.transform.up * (float)(shootTextComponent.initializedVerticalPositionOffset + shootTextComponent.yMoveOffeset);

                shootTextComponent.transform.position = shootTextCreatPosition;
            }
        ...
        if ((updateCreatTempTime -= deltaTime) <= 0)
        {
            ...
            while (waitShootTextGroup.Count > 0 && count-- > 0)
            {
                ...
                tempObj.transform.SetParent(ShootTextCanvas.transform, false);
                ...
                {
                    tempObj.transform.forward = ShootTextCamera.transform.forward;
                }
            }
        }
    ...
    public void CreatShootText(float resScale, bool isNeedShowPreOperator, TextMoveType textMoveType, int showValue, Func<Vector3> getShootTextTopPoint, Func<Vector3> getShootTextButtomPoint)
    {
        if (textMoveType == TextMoveType.None)
        {
            textMoveType = this.textMoveType;
        }
        CreatShootText(isNeedShowPreOperator, showValue, textMoveType, this.delayMoveTime, this.initializedVerticalPositionOffset * resScale, this.initializedHorizontalPositionOffset * resScale, getShootTextTopPoint, getShootTextButtomPoint);
    }
    ...
    private void BuildNumber(ShootTextInfo shootTextInfo, Transform parent)
    {
        ...
        if (isNeedShowPreOperator)
        {
            ...
            operatorObj.transform.localPosition = Vector3.zero;
            operatorObj.transform.localEulerAngles = Vector3.zero;
        }
    ...
    private void DealOneByOne(Transform parent, int showValue)
    {
        ...
        for (int i = this.tmp.Count - 1; i >= 0; i--)
        {
            ...
            numberObj.transform.localEulerAngles = Vector3.zero;
        }
        this.tmp.Clear();
    }
    ...
    private double ModelInScreenHigh(ShootTextComponent shootTextComponent)
    {
        ...
        if (this.UITextCamera != this.ShootTextCamera)
        {
            topValue = shootTextComponent.getShootTextTopPoint().y;
            bottomValue = shootTextComponent.getShootTextButtomPoint().y;
        }
        else
        {
            topValue = ShootTextCamera.WorldToScreenPoint(shootTextComponent.getShootTextTopPoint()).y;
            bottomValue = ShootTextCamera.WorldToScreenPoint(shootTextComponent.getShootTextButtomPoint()).y;
        }
