...
namespace ET.Client
{
    ...
    public class EntryEvent3_InitClient: AEvent<Scene, ET.EventType.EntryEvent3>
    {
        ...
        public static async ETTask<bool> ChkHotUpdateAsync(Scene clientScene, bool isAutoInToDlgUpdate = true, bool isShowTipWhenNeedUpdate = true)
        {
            ...
            {
                await ET.Client.UIManagerHelper.ShowUpdate(clientScene);
            }
        ...
        public static async ETTask DoAfterChkHotUpdate(Scene clientScene)
        {
            ...
            {
                // 打开热更界面
                await UIManagerHelper.GetUIComponent(clientScene).ShowWindowAsync<DlgLoading>();
                await Game.GetExistSingleton<ConfigComponent>().LoadAsync((progress) =>
                {
                    EventSystem.Instance.Publish(clientScene, new ClientEventType.LoadingProgress() { curProgress = progress });
                });
            }
            ...
            if (clientScene.GetComponent<AttributionModelSDKManagerComponent>() == null)
            {
                clientScene.AddComponent<AttributionModelSDKManagerComponent>();
            }
            if (clientScene.GetComponent<EventLoggingSDKComponent>() == null)
            {
                clientScene.AddComponent<EventLoggingSDKComponent>();
            }
            if (clientScene.GetComponent<AdvertSDKManagerComponent>() == null)
            {
                clientScene.AddComponent<AdvertSDKManagerComponent>();
            }
        ...
        private static async ETTask<bool> HotUpdateAsync(Scene clientScene, bool isShowTipWhenNeedUpdate = true)
        {
            ...
            if (ET.Client.GlobalComponent.Instance.ChkIsUpdateFinished() == false)
            {
        ...
        private static async ETTask<bool> DownloadPatch(Scene clientScene, YooAsset.UpdatePackageManifestOperation updatePackageManifestOperation)
        {
            ...
            int errorCode = await ResComponent.Instance.DonwloadWebFilesAsync(
                ...
                {
                    // 更新进度条
                    EventSystem.Instance.Publish(clientScene, new ClientEventType.OnPatchDownloadProgress() { TotalDownloadCount = totalDownloadCount, CurrentDownloadCount = currentDownloadCount, TotalDownloadSizeBytes = totalDownloadBytes, CurrentDownloadSizeBytes = currentDownloadBytes });
                },
                ...
                {
                    // 下载失败
                    EventSystem.Instance.Publish(clientScene, new ClientEventType.OnPatchDownlodFailed() { FileName = fileName, Error = error });
                },
        ...
        {
            // Hide the dlg update window first before reloading.
            UIManagerHelper.HideUpdate(scene);

            // Wait for all hidden before reloading.
            await TimerComponent.Instance.WaitAsync(200);

            Log.Debug($"[{Time.frameCount}] Reload all");
            await GameObject.Find("/Init").GetComponent<Init>().Restart();
            Log.Debug($"[{Time.frameCount}] All reloaded");
        }
