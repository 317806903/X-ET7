...
namespace ET
{
    ...
    public static class PutHomeComponentSystem
    {
        ...
        public static bool InitHomeByPlayer(this PutHomeComponent self, long playerId, string unitCfgId, float3 homePos, float3 forward)
        {
            GamePlayTowerDefenseComponent gamePlayTowerDefenseComponent = self.GetGamePlayTowerDefense();
            ...
            Unit homeUnit = self.CreateHome(unitCfgId, homePos, forward, (int)hp, (int)hp, homeTeamFlagType);
        ...
        public static Unit CreateHome(this PutHomeComponent self, string unitCfgId, float3 pos, float3 forward, int maxHp, int hp, TeamFlagType teamFlagType)
        {
            return GamePlayTowerDefenseHelper.CreateHome(self.DomainScene(), unitCfgId, pos, forward, maxHp, hp, teamFlagType);
        }
        ...
        {
            Dictionary<TeamFlagType, long> homeUnitList = self.GetHomeUnitList();
            foreach (var item in homeUnitList)
            {
                long homeUnitId = item.Value;
                Unit homeUnit = UnitHelper.GetUnit(self.DomainScene(), homeUnitId);
                NumericComponent numericComponent = homeUnit.GetComponent<NumericComponent>();
                string homeCfgId = homeUnit.CfgId;
                float3 homePos = homeUnit.Position;
                float3 homeForward = homeUnit.Forward;
                int maxHp = numericComponent.GetAsInt(NumericType.MaxHp);
                int hp = numericComponent.GetAsInt(NumericType.Hp);
                self.RecordHomeInfo[homeUnitId] = (homeCfgId, homePos, homeForward, maxHp, hp);
            }
        }

        public static void ResetHomeHp(this PutHomeComponent self, long homeUnitId, float addCurHp, float addMaxHp)
        {
            if (self.RecordHomeInfo.ContainsKey(homeUnitId) == false)
            {
                return;
            }
            var recordInfo = self.RecordHomeInfo[homeUnitId];
            string homeCfgId = recordInfo.homeCfgId;
            float3 homePos = recordInfo.homePos;
            float3 homeForward = recordInfo.homeForward;
            float maxHp = recordInfo.maxHp + addMaxHp;
            float hp = recordInfo.hp + addCurHp;
            self.RecordHomeInfo[homeUnitId] = (homeCfgId, homePos, homeForward, (int)maxHp, (int)hp);
        }
        ...
        public static void RecoverHomeHp(this PutHomeComponent self, int recoverAddHp)
        {
            ...
            {
                (string homeCfgId, float3 homePos, float3 homeForward, int maxHp, int curHp) = self.RecordHomeInfo[homeUnitId.Value];
                TeamFlagType teamFlagType = homeUnitId.Key;
                curHp = math.min(recoverAddHp + curHp, maxHp);
                Unit newHomeUnit = self.CreateHome(homeCfgId, homePos, homeForward, maxHp, curHp, teamFlagType);
