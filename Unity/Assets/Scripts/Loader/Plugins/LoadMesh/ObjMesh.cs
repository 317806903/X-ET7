...
namespace ET.LoadMesh
{
    public class ObjMesh
    {
        ...
        public ObjMesh LoadFromObj(string objText)
        {
            ...
            foreach (string line in allLines)
            {
                ...
                switch (chars[0])
                {
                    ...
                    case "vn":
                        //处理法线
                        this.normalArrayList.Add(new Vector3(ConvertToFloat(chars[1]),
                            ConvertToFloat(chars[2]),
                            ConvertToFloat(chars[3])));
                        break;
                    case "vt":
                        //处理UV
                        this.uvArrayList.Add(new Vector3(ConvertToFloat(chars[1]),
                            ConvertToFloat(chars[2])));
                        break;
                    case "f":
                        //处理面
        ...
        private void Combine()
        {
            ...
            MultiDictionary<float, float, List<int>> multDic = new();
            for (int i = 0; i < faceVertexNormalUV.Count; i++)
            {
                Vector3 vec = faceVertexNormalUV[i];
                if (vec.Equals(Vector3.zero) == false)
                {
                    if (multDic.TryGetValue(vec.x, vec.y, out var hashSet) == false)
                    {
                        hashSet = new();
                        multDic.Add(vec.x, vec.y, hashSet);
                    }
                    hashSet.Add(i);
                }
            }
            ...
            for (int i = 0; i < faceVertexNormalUV.Count; i++)
            {
                ...
                if (vec.Equals(Vector3.zero) == false)
                {
                    ...
                    foreach (int index in hashSet)
                    {
                        ...
                        {
                            faceVertexNormalUV[index] = Vector3.zero;
                        }
                    }
                    //用一个索引来作为字典的键名，这样它可以代替对应列表内所有索引
                    toCambineList.Add(i, SameIndexList);
            ...
            foreach (KeyValuePair<int, ArrayList> IndexTtem in toCambineList)
            {
                ...
                try
                {
                    ...
                    if ((int)VectorTemp.x - 1 >= vertexArrayList.Count)
                    {
                        int i2 = 0;
                    }
                    //给顶点数组赋值
