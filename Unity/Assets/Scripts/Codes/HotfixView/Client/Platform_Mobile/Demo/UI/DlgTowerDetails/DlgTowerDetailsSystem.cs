...
namespace ET.Client
{
	...
	public static class DlgTowerDetailsSystem
	{
		...
		public static async ETTask ShowDetails(this DlgTowerDetails self)
		{
			...
			string unitCfgId = ET.ItemHelper.GetTowerItem2UnitCfgId(self.curTowerCfgId);
			int unitCfgLevel = ET.ItemHelper.GetTowerItemLevel(self.curTowerCfgId);
			string propertyType = UnitCfgCategory.Instance.Get(unitCfgId).PropertyType;
			self.ShowAttribute(propertyType, unitCfgLevel);
		...
		public static void ShowUpgradeInfo(this DlgTowerDetails self)
		{
			if (ET.Client.UIManagerHelper.ChkIsDebug())
			{
				...
				{
					ET.Client.ItemHelper.UpgradeItem(self.DomainScene(), self.curTowerCfgId).Coroutine();
				});
			}
			else
			{
        ...
        public static async ETTask AddCardWhenDebug(this DlgTowerDetails self)
        {
	        if (GlobalConfig.Instance.dbType == DBType.NoDB)
	        {
		        ...
		        bool isHave = playerBackPackComponent.ChkItemExist(self.baseTowerCfgId);
		        if (isHave)
		        {
			        return;
		        }
