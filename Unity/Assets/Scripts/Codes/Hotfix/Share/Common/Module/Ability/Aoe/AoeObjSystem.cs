...
namespace ET.Ability
{
    ...
    public static class AoeObjSystem
    {
        ...
        public static void EventHandler(this AoeObj self, AoeActionCall aoeActionCall, ref ActionContext actionContext)
        {
            ...
            SelectHandle selectHandle = ET.Ability.SelectHandleHelper.CreateSelectHandleWhenAoe(self.GetUnit(), self.aoeTargetCondition.SelectObjectTeamFlagType, aoeActionCall.AoeSelectObjectType);
            if (selectHandle == null)
            {
                return;
            }
            ET.Ability.ActionHandlerHelper.DoActionTriggerHandler(self.GetUnit(), self.GetUnit(), aoeActionCall.DelayTime, aoeActionCall.ActionIds, aoeActionCall.FilterCondition1, aoeActionCall.FilterCondition2, selectHandle, null, ref actionContext);
        }
        ...
        public static void ChkAoeEnterOrExist(this AoeObj self)
        {
            ...
            SelectHandle curSelectHandle = ET.Ability.SelectHandleHelper.GetSelectHandleWithSelectObjectType(curUnit, self.aoeTargetCondition.SelectObjectTeamFlagType, self.aoeTargetCondition.SelectObjectUnitTypeBase, list);
            (bool bRet1, bool isChgSelect1, SelectHandle newSelectHandle1) = UnitConditionHandleHelper.ChkConditionWhenFilter(self.GetUnit(), curSelectHandle, self.aoeTargetCondition.FilterCondition1, ref self.actionContext);
            if (isChgSelect1)
            {
            ...
            if (bRet1)
            {
                (bool bRet2, bool isChgSelect2, SelectHandle newSelectHandle2) = UnitConditionHandleHelper.ChkConditionWhenFilter(self.GetUnit(), curSelectHandle, self.aoeTargetCondition.FilterCondition2, ref self.actionContext);
                if (isChgSelect2)
                {
