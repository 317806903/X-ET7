        ...
        public static void FixedUpdate(this TimelineObj self, float fixedDeltaTime)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                )
                {
                    ...
                    bool bRet = ET.Ability.ActionHandlerHelper.DoActionTriggerHandler(self.GetUnit(), self.GetUnit(), timelineNode.DelayTime, timelineNode.ActionIds, timelineNode.FilterCondition1, timelineNode.FilterCondition2, curSelectHandle, null, ref self.actionContext);
                    if (bRet)
                    {
                        foreach (string actionId in timelineNode.ActionIds)
                        {
