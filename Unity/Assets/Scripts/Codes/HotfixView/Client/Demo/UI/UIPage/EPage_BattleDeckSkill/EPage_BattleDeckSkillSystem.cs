...
namespace ET.Client
{
	...
	public static class EPage_BattleDeckSkillSystem
	{
		...
		public static async ETTask ChkPointUp(this EPage_BattleDeckSkill self)
		{
			...
			PlayerBattleSkillComponent playerBattleSkillComponent = await ET.Client.PlayerCacheHelper.GetMyPlayerBattleSkill(self.DomainScene(), false);
			playerBattleSkillComponent.ReplaceBattleSkillItemCfgId(replaceIndex, moveItemCfgId);
			await ET.Client.PlayerCacheHelper.SaveMyPlayerModel(self.DomainScene(), PlayerModelType.BattleSkill, null);
		...
		public static async ETTask AddSkillsWhenDebug(this EPage_BattleDeckSkill self)
		{
			if (GlobalConfig.Instance.dbType == DBType.NoDB)
			{
				...
				{
					bool isExist = playerBackPackComponent._ChkItemExist(SkillCfgId);
					if (isExist == false)
					{
						isChg = true;
