...
namespace MirrorVerse.UI.MirrorSceneDefaultUI
{
    ...
    public delegate void OnMenuQuitScene();
    public delegate void OnMenuJoinByExistScene();
    public delegate void OnMenuJoinByQRCode(string sQRCodeInfo);

    public delegate (bool, string) OnGetQRCodeInfo();
    public class DefaultUI : MonoBehaviour
    {
        ...
        public event OnMenuQuitScene onMenuQuitScene = delegate { };
        public event OnMenuJoinByExistScene onMenuJoinByExistScene = delegate { };

        public event OnMenuJoinByQRCode onMenuJoinByQRCode = delegate { };
        public event OnGetQRCodeInfo onGetQRCodeInfo = null;
        ...
        public void OnMarkerDetected(StatusOr<MarkerInfo> markerInfo, StatusOr<Pose> markerPose, StatusOr<Pose> localizedPose)
        {
            if (markerInfo.Status.IsOk)
            {
                ...
                string sQrCodeInfo = "";
                if (markerInfo.HasValue && markerInfo.Value.extraData != null)
                {
                    sQrCodeInfo = System.Text.Encoding.UTF8.GetString(markerInfo.Value.extraData);
                }
                this.onMenuJoinByQRCode(sQrCodeInfo);
            }
        ...
        {
            string sQRCodeInfo = "";
            if (this.onGetQRCodeInfo != null)
            {
                bool isReady;
                (isReady, sQRCodeInfo) = this.onGetQRCodeInfo();
                if (isReady == false)
                {
                    return false;
                }
            }
            Status status = _api.ShowMarker(sQRCodeInfo);
            if (status.IsOk)
            {
                return true;
            }
        ...
        public bool ChkSceneStatusCompleted()
        {
            SceneStatus sceneStatus = _api.GetSceneStatus();
            if (sceneStatus == SceneStatus.Completed)
            {
                return true;
            }
            return false;
