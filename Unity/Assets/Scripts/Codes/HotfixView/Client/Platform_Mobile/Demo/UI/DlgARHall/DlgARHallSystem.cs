    ...
    public static class DlgARHallSystem
    {
        ...
        public static async ETTask ShowWindow(this DlgARHall self, ShowWindowData contextData = null)
        {
            ...
            else
            {
                ...
                self.arSceneMeshId = _DlgARHall_ShowWindowData.arSceneMeshId;
        ...
        public static void HideWindow(this DlgARHall self)
        {
        ...
        public static async ETTask InitArSession(this DlgARHall self)
        {
            ...
            EventSystem.Instance.Publish(self.DomainScene(), new ClientEventType.NoticeEventLoggingStart()
            {
                // 准备阶段结束计时开始
        ...
        public static async ETTask TriggerJoinScene(this DlgARHall self)
        {
            ...
            self.arSceneMeshId = roomComponent.arSceneMeshId;
            ...
            {
                var arRoomInfo = await RoomHelper.GetARRoomInfoAsync(self.DomainScene(), self.roomId, false);
                ARMeshType arMeshType = arRoomInfo.arMeshType;
                if (arMeshType == ARMeshType.FromRemoteURL_AliyunOSS)
                {
                    await ET.Client.ARSessionHelper.CreateMeshFromAliyun(self.DomainScene(), arRoomInfo.arSceneMeshId);
                }
                else if (arMeshType == ARMeshType.FromClientObj)
                {
                    byte[] bytes = await ET.GamePlayHelper.ReadMeshFileBytes(arRoomInfo.arSceneMeshId);
                    if (bytes == null)
                    {
                        arRoomInfo = await RoomHelper.GetARRoomInfoAsync(self.DomainScene(), self.roomId, true);
                        bytes = arRoomInfo.arMeshBytes;
                        await ET.GamePlayHelper.WriteMeshFileBytes(arRoomInfo.arSceneMeshId, bytes);
                    }
                    await ET.Client.ARSessionHelper.CreateMeshFromClientObj(self.DomainScene(), bytes);
                }
            }
        ...
        public static async ETTask CreateRoom(this DlgARHall self)
        {
            ...
            {
                Log.Error($"GamePlayBattleLevelCfgCategory.Instance.Contain({self.roomTypeInfo.gamePlayBattleLevelCfgId}) == false self.roomTypeInfo[{self.roomTypeInfo}]");
                return;
            }
