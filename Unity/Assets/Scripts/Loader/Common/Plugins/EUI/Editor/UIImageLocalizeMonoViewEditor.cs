...
public class UIImageLocalizeMonoViewEditor : Editor
{
    ...
    public void InitCurPath()
    {
        ...
        {
            curPath = this.m_Target.GetImagePathWhenEditor("None", Path.stringValue, false);
        }
        else
        {
    ...
    public (bool bRet, string relativePath) ChkIsValidate(string path)
    {
        ...
        if (path.ToLower().EndsWith(".png")||path.ToLower().EndsWith(".jpg"))
        {
            ...
            if (path.StartsWith($"{UIImageLocalizeMonoView.UI3D_MultiLanguagePath}/CN/"))
            {
                return (true, path.Replace($"{UIImageLocalizeMonoView.UI3D_MultiLanguagePath}/CN/", ""));
            }
            if (path.StartsWith($"{UIImageLocalizeMonoView.UI3D_MultiLanguagePath}/EN/"))
            {
                return (true, path.Replace($"{UIImageLocalizeMonoView.UI3D_MultiLanguagePath}/EN/", ""));
            }
            if (path.StartsWith($"{UIImageLocalizeMonoView.UI3D_MultiLanguagePath}/TW/"))
            {
                return (true, path.Replace($"{UIImageLocalizeMonoView.UI3D_MultiLanguagePath}/TW/", ""));
            }
